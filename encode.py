...
def main(args=None):
    ...
    if isinstance(snapshot_stage, int):
        enc_figname = os.path.join(models_dir, 'ep%04d_%s.pth.tar' % (snapshot_stage, encoder.name))
    else:
        enc_figname = os.path.join(models_dir, '%s_%s.pth.tar' % (snapshot_stage, encoder.name))
    ...
    fname = os.path.join(logs_dir, '%s_%s_stage-%s_encoded.npz'%(dataset_name, ('train' if args.dataset_train else 'test'), (snapshot_stage if isinstance(snapshot_stage, str) else 'ep%04d' % snapshot_stage)))
